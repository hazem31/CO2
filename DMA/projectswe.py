    ...
    def view(self):
        if(self.session.type == "Student"):
            ...
            if(requested_ID != "choose Student ID"):
                ...
                item = self.table_for_stu.item(0, 0)
                item.setText(str(course.file.set_index('studentID')["MidtermResult"][int(self.sel_course_stu.currentText())]))

                item = self.table_for_stu.item(0, 1)
                item.setText(str(course.file.set_index('studentID')["Attendence"][int(self.sel_course_stu.currentText())]))

                item = self.table_for_stu.item(0, 2)
                item.setText(str(course.file.set_index('studentID')["Project"][int(self.sel_course_stu.currentText())]))

                item = self.table_for_stu.item(0, 3)
                item.setText(str(course.file.set_index('studentID')["assigiment"][int(self.sel_course_stu.currentText())]))

                item = self.table_for_stu.item(0, 4)
                item.setText(str(course.file.set_index('studentID')["final"][int(self.sel_course_stu.currentText())]))
