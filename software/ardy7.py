...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.sel_diagram.setItemText(1, _translate("MainWindow", "Attendence histogram"))
        self.sel_diagram.setItemText(2, _translate("MainWindow", "MidtermResult histogram"))
        self.sel_diagram.setItemText(3, _translate("MainWindow", "Project histogram"))
        self.sel_diagram.setItemText(4, _translate("MainWindow", "final histogram"))
        self.sel_diagram.setItemText(5, _translate("MainWindow", "assigiment histogram"))
    ...
    def plot(self):
        ...
        if (requested_dia != "diagram"):
            course_man = pd.read_csv(str(self.session.professor.courses[0]) + ".csv")
            his = requested_dia.split()[0]
            print(his)
            if(his == "MidtermResult"):
                bins = np.arange(0,course_man['MidtermResult'].max()+4,4)
                bins = [0,4,8,12,16,20]
                plt.figure()
                plt.hist(data=course_man , x='MidtermResult',bins=bins)
                plt.xticks(bins)
                plt.ylabel("Midterm")
                plt.show()
            elif(his == "Attendence"):
                bins = [0, 1, 2, 3, 4, 5]
                plt.figure()
                plt.hist(data=course_man, x='Attendence', bins=bins)
                plt.xticks(bins)
                ...
                bins = [0, 1, 2, 3, 4, 5]
                plt.figure()
                plt.hist(data=course_man, x='assigiment', bins=bins)
                plt.xticks(bins)
                plt.ylabel("assigiment")
                plt.show()
            elif (his == "Project"):
                bins = [0, 3, 6, 9, 12, 15]
                plt.figure()
                plt.hist(data=course_man, x='Project', bins=bins)
                plt.xticks(bins)
                plt.ylabel("Project")
                plt.show()
            elif (his == "final"):
                bins = [0, 2, 4, 6, 8, 10]
                plt.figure()
                plt.hist(data=course_man, x='final', bins=bins)
                plt.xticks(bins)
                plt.ylabel("final")
                plt.show()
